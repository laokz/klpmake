...
static size_t rv_search_lo12(Elf_Data *reladata, GElf_Rela *rela, size_t rela_count, struct orig_elf_t *para)
{
	GElf_Rela lo12_rela;
	size_t idx;
	Elf_Scn *scn;
			...
			for (int m = klpsyms->klpsym_count - 1; m >= 0; m--)
				if (GELF_R_SYM(rela.r_info) == klpsyms->u[m].symindex) {
					...
					if (para->arch == EM_RISCV && (
						...
						E(p = malloc(sizeof(struct rela_klpsym_t)), NULL);
						p->relasec_index = i;
						p->entry_index = rv_search_lo12(data, &rela, sh.sh_size / sh.sh_entsize, para);
						p->relasec_name = name;
						p->rela = rela;
						p->next = klpsyms->u[m].head;
						klpsyms->u[m].head = p;
...
static void scan_newsecs(Elf *relf, struct klpsyms_t *klpsyms)
{
	...
	E(klpsyms->secs = malloc(sizeof(struct klp_rela_t) * klpsyms->newsec_count), NULL);
...
static void copy_sec_data_meta(GElf_Shdr *oldsh, GElf_Shdr *newsh, Elf_Data *olddata, Elf_Data *newdata, size_t new_buflen)
{
	char *buf;
 ...
 */
static void modify_strtab(Elf *welf, GElf_Shdr *sh, Elf_Data *data, struct klpsyms_t *klpsyms, struct orig_elf_t *para)
{
	GElf_Shdr newsh;
	Elf_Data newdata;
...
static void modify_shstrtab(Elf *welf, GElf_Shdr *sh, Elf_Data *data, struct klpsyms_t *klpsyms, struct orig_elf_t *para)
{
	GElf_Shdr newsh;
	Elf_Data newdata;
...
static void modify_symtab(Elf *welf, GElf_Shdr *sh, Elf_Data *data, struct klpsyms_t *klpsyms, struct orig_elf_t *para)
{
	GElf_Sym sym;
...
static void modify_relasec(Elf *welf, GElf_Shdr *sh, Elf_Data *data, struct klpsyms_t *klpsyms, struct orig_elf_t *para)
{
	GElf_Rela rela;
	struct rela_klpsym_t *p;
	...
	for (int i = 0; i < sh->sh_size / sh->sh_entsize; i++) {
		for (int j = 0; j < klpsyms->klpsym_count; j++) {
			...
			while (p) {
				...
				if (p->relasec_index == para->current_sec_index && p->entry_index == i)
					goto outer;
				p = p->next;
			}
		}
...
static void create_klp_rela_sec(Elf *welf, struct klpsyms_t *klpsyms, struct orig_elf_t *para)
{
	Elf_Scn *wscn;
	GElf_Shdr newsh;
	Elf_Data *wdata;
	...
	for (int i = 0; i < klpsyms->newsec_count; i++) {
		...
		copy_sec_data_meta(&klpsyms->secs[i].sh, &newsh, &klpsyms->secs[i].data, wdata, klpsyms->secs[i].data.d_size);
...
static void modify_obj(const char *obj, const char *newobj, struct klpsyms_t *klpsyms)
{
	Elf *relf, *welf;
	int rfd, wfd;
	Elf_Data *rdata;
